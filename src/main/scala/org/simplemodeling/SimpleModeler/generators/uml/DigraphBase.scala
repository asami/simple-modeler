...
class DigraphBase(
  ...
  def node_color(anObject: MElement, node: GVNode) {
    anObject match {
      case _: MStoryObject => {
        node.style = "filled"
  ...
  def name_compartment(anObject: MElement, node: GVNode) {
  ...
  def attribute_compartment(anObject: MObject, node: GVNode) {
    val comp = new GVCompartment
  ...
  def operation_compartment(anObject: MObject, node: GVNode) {
  }
...
//      }
  ...
  def kind_compartment(aPowertype: MPowertype, node: GVNode) {
    val comp = new GVCompartment
...
//        node.shape = "hexagon"
//      }
      case story: MStoryObject => node.shape = "ellipse"
      case _ => {}
    }
    node_color(anObject, node)
    name_compartment(anObject, node)
  ...
  final def addTraitRelationship(aSourceId: String, aTargetId: String, aRel: MTraitRef) {
    addTraitRelationship(aSourceId, aTargetId, aRel, false)
  }

  final def addDerivedTraitRelationship(aSourceId: String, aTargetId: String, aRel: MTraitRef) {
    addTraitRelationship(aSourceId, aTargetId, aRel, true)
  }

  final def addTraitRelationship(aSourceId: String, aTargetId: String, aRel: MTraitRef, aDerived: Boolean) {
    val edge = make_trait_edge(aSourceId, aTargetId)
    graph.edges += edge
  }

  final def addPlainTraitRelationship(aSourceId: String, aTargetId: String, aRel: MTraitRef) {
    val edge = make_trait_edge(aSourceId, aTargetId)
    graph.edges += edge
  }

  final def addSimpleTraitRelationship(aSourceId: String, aTargetId: String, aRel: MTraitRef) {
    val edge = make_trait_edge(aSourceId, aTargetId)
    graph.edges += edge
  }
   ...
   */
  final def addStateMachineRelationship(aSourceId: String, aTargetId: String, aRel: MStateMachineRef) {
    addStateMachineRelationship(aSourceId, aTargetId, aRel, false)
  }

  final def addDerivedStateMachineRelationship(aSourceId: String, aTargetId: String, aRel: MStateMachineRef) {
    addStateMachineRelationship(aSourceId, aTargetId, aRel, true)
  }

  final def addStateMachineRelationship(aSourceId: String, aTargetId: String, aRel: MStateMachineRef, aDerived: Boolean) {
    val edge = make_statemachine_edge(aSourceId, aTargetId)
    edge.taillabel = aRel.name
    graph.edges += edge
  }

  final def addPlainStateMachineRelationship(aSourceId: String, aTargetId: String, aRel: MStateMachineRef) {
    val edge = make_statemachine_edge(aSourceId, aTargetId)
    graph.edges += edge
  }

  final def addSimpleStateMachineRelationship(aSourceId: String, aTargetId: String, aRel: MStateMachineRef) {
    val edge = make_statemachine_edge(aSourceId, aTargetId)
    graph.edges += edge
  }
   ...
   */
  final def addAssociationClassRelationship(aSourceId: String, targets: Seq[(String, MAssociation)]) {
    addAssociationClassRelationship(aSourceId, targets, false)
  }

  final def addDerivedAssociationClassRelationship(aSourceId: String, targets: Seq[(String, MAssociation)]) {
    addAssociationClassRelationship(aSourceId, targets, true)
  }

  final def addAssociationClassRelationship(aSourceId: String, targets: Seq[(String, MAssociation)], aDerived: Boolean) {
  ...
  final def addPlainAssociationClassRelationship(aSourceId: String, targets: Seq[(String, MAssociation)]) {
    build_assoc_class_edge(aSourceId, targets)
  }

  final def addSimpleAssociationClassRelationship(aSourceId: String, targets: Seq[(String, MAssociation)]) {
    build_assoc_class_edge(aSourceId, targets)
  }

  private def build_assoc_class_edge(aSourceId: String, targets: Seq[(String, MAssociation)]): Seq[(GVEdge, MAssociation)] = {
    ...
    def maketargetlines(pid: String, targets: Seq[(String, MAssociation)]) = {
...
//  }
  ...
  private def get_stereotype_color(elem: MElement): String = {
    // "extension") "#ffffff"
    // "") "#f8fbf8" /// 白磁 はくじ
    elem match {
      // Business
      case _: MBusinessActor => "#954e2a" // 柿茶かきちゃ
      case _: MBusinessResource => "#007bbb" // 紺碧こんぺき
      case _: MBusinessEvent => "#c9171e" // 深緋こきひ
      case _: MBusinessUsecase => "#4f455c" // 濃鼠 こいねず
      case _: MBusinessTask => "#4f455c" // 濃鼠 こいねず
      // Requirement
      case _: MRequirementUsecase => "#705b67" // 葡萄鼠 ぶどうねずみ
      case _: MRequirementTask => "#705b67" // 葡萄鼠 ぶどうねずみ
      // Domain
      case _: MDomainActor => "#f39800" // 金茶 きんちゃ
      case _: MDomainRole => "#e49e61" // 小麦色 こむぎいろ
      case _: MDomainResource => "#38a1db" // 露草色 つゆくさいろ
      case _: MDomainEvent => "#e2041b" // 猩々緋 しょうじょうひ
      case _: MDomainSummary => "#cc7eb1" // 菖蒲色 しょうぶいろ "#a6a5c4" // 藤鼠 ふじねず "#84a2d4" // 青藤色 あおふじいろ // "#674196" // 菖蒲色 しょうぶいろ
      case _: MDomainAssociationEntity => "#fef263" // 黄檗色きはだいろ // "#c89932" // 山吹茶やまぶきちゃ
      case _: MDomainEntity => "#c1e4e9" // 白藍 しらあい
      case _: MDomainTrait => "#b3ada0" // 利休白茶
      // Domain(implicit)
//      case _: MEntityPart => "#c1e4e9" // 白藍 しらあい
      case _: MEntity => "#c1e4e9" // 白藍 しらあい
      ...
      // System(implicit)
      case _: MDataSource => "#d4dcda" // 薄雲鼠うすくもねず
      case _: MDataSet => "#b3ada0" // 利休白茶りきゅうしろちゃ
      case _: MFlow => "#d8e698" // 若菜色わかないろ
      case _ => ""
    }
  }
...
*/
  ...
  private def get_stereotype_fontcolor(elem: MElement): Option[String] = {
    elem match {
      case _: MStoryObject => Some("white")
      case _: MBusinessEntity => Some("white")
      case _ => None
    }
  }
