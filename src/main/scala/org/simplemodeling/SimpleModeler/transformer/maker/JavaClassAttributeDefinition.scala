...
/*
 * Java Class Attribute Definition
 * 
 * @since   Jun. 21, 2011
 *  version Aug.  7, 2011
 *  version Dec. 14, 2011
...
class JavaClassAttributeDefinition(
  ...
  jmaker: JavaMaker
) extends GenericClassAttributeDefinition(ctx, model, aspects, anattr, jowner)
    with JavaMakerHolder
    with JavaClassAttributeDefinitionHelper
    with JavaClassAttributeDefinitionExtension
    with JavaClassAttributeCodeUtils {
  /**
   ...
   */
  def isVoucher = jowner.isVoucher
...
//      jm_assign_this(attrName, "%s.getFactory().create%s(json)", owner.factoryName, e.entity.voucherName)
//      jm_return_this
//    } 
  }
...
//          jm_assign("doc.%s_now".format(varName), "false")
        ...
        case p: PEntityPartType => {
          jm_pln("doc.%s = this.%s.make_voucher();", varName, refVarName)
        }
        ...
        case p: PEntityPartType => {
          ...
          jm_if_not_null("this." + partVarName) {
            jm_for(java_element_type() + " element: " + partVarName) {
              jm_pln("doc.%s.add(element.make_voucher());", attrName)
            }
          }
        }
//        case v: PByteType    => update("%s.byteValue()")
...
/*
          ...
          def deep {
            ...
            jm_if_not_null(refVarName) {
              jm_assign("doc.%s".format(varName), "%s.make_voucher()", refVarName)
            }
          }
          ...
          def shallow {
            jm_pln("fill_%s();".format(attrName))
            jm_if_not_null(refVarName) {
              jm_assign("doc.%s".format(varName), "%s.make_voucher_shallow()", refVarName)
            }
          }
          ...
          if (is_owned_property()) {
            ...
            jm_if_not_null(refVarName) {
              get_id
              jm_pln("doc.%s = %s.make_voucher();".format(varName, refVarName))
            }
 ...
 */
  def method_voucher_make_single_entity_simple(e: PEntityType) {
    jm_pln("doc.%s = this.%s.make_voucher();", varName, varName)
  }
...
/*
          ...
          def deep {
            ...
            jm_for("%s entity: %s".format(e.entity.name, refVarName)) {
              jm_pln("doc.%s.add(entity.make_voucher());".format(varName))
            }
          }
          ...
          def shallow {
            jm_pln("fill_%s();".format(attrName))
            jm_for("%s entity: %s".format(e.entity.name, refVarName)) {
              jm_pln("doc.%s.add(entity.make_voucher_shallow());".format(varName))
            }
          }
