...
abstract class GenericClassDefinition(
  ...
  def this(c: PContext, m: PModel, o: PObject) = this(c, m, Nil, o)

  set_Recorder(pContext.recorder)

  val voucherName = pobject match {
    case e: PEntity => e.voucherName
    case _ => ""
  }
  def isVoucherOwner = StringUtils.isNotBlank(voucherName)
  // val modelObject: MObject = pobject.modelObject
  def packageObject: PPackage = model.packageOfObject(pobject)
  ...
  // val modelEntityOption: Option[MEntity] = modelObject match {
  //   case entity: MEntity => Some(entity)
  //   case _ => None
  // }
  // def modelEntity: MEntity = modelEntityOption.get
  // def modelPackageOption: Option[MPackage] = pobject.modelPackage
  // def modelPackage: MPackage = pobject.modelPackage.get
...
*/
  ...
  def wholeAttributeDefinitions(o: PObject): List[ATTR_DEF] = {
    val a = parentAttributeDefinitions(o) ::: implementsAttributeDefinitions(o)
    _ordering(_cleansing(a))
  }

  def parentAttributeDefinitions(o: PObject): List[ATTR_DEF] = {
    o.getBaseObject(model).map(wholeAttributeDefinitions) | Nil
  }

  def implementsAttributeDefinitions(o: PObject): List[ATTR_DEF] = {
    val a = attributeDefinitions(o) ::: traitsAttributeDefinitions(o) ::: associationClassAttributeDefinitions(o)
    _ordering(_cleansing(a))
  }

  def attributeDefinitions(o: PObject): List[ATTR_DEF] = {
    val a = o.attributes.map(attribute).toList
    _ordering(a)
  }

  def traitsAttributeDefinitions(o: PObject): List[ATTR_DEF] = {
  ...
  def associationClassAttributeDefinitions(o: PObject): List[ATTR_DEF] = {
  ...
  private def _cleansing(attrs: List[ATTR_DEF]): List[ATTR_DEF] = {
    attrs.foldLeft((nil[ATTR_DEF], Set.empty[String]))((a, x) => {
      if (a._2.contains(x.attr.name)) {
        record_warning(s"「${name}」で属性・関連「${x.attr.name}」の重複があります。")
        a
      } else {
...
//  aspects.foreach(_.open(_maker))
  ...
  protected def class_open_voucher {
  }
  ...
  protected def attribute_variables_BackReference(source: MObject, assoc: MAssociation) {}
  ...
  protected def package_variables_simplemodel {
    for (child <- packageObject.elements) {
//    for (pkg <- pobject.modelPackage; child <- pkg.children) {
      ...
      //   case voucher: SMDomainVoucher   => package_variables_Voucher(voucher)
  ...
  // protected def package_variables_Voucher(voucher: SMDomainVoucher) {
  // }
  ...
  val package_variables_platformmodel_visitor = new PObjectFunction[Unit] {
    override protected def apply_EntityEntity(entity: PEntity) {
      package_variables_EntityEntity(entity)
    }

    override protected def apply_PackageEntity(pkg: PPackage) {
      package_variables_PackageEntity(pkg)
    }

    override protected def apply_ObjectEntity(pkg: PObject) {
    }
  }
...
/*
    ...
    if (!children.isEmpty) {
      val f = new PObjectFunction[Unit] {
...
*/
  ...
  protected def package_variables_EntityEntity(entity: PEntity) {
  }

  protected def package_variables_PackageEntity(pkg: PPackage) {
  }
   ...
   * Voucher
   */
  protected def voucher_methods {
    voucher_methods_make
    voucher_methods_update
    voucher_methods_Extension
  }

  protected def voucher_methods_make {
  }

  protected def voucher_methods_update {
  }

  protected def voucher_methods_Extension {
  }
  ...
  protected def package_methods_simplemodel {
    // for (pkg <- pobject.modelPackage; child <- pkg.children) {
      ...
      //   case voucher: SMDomainVoucher   => package_methods_Voucher(voucher)
  ...
  // protected def package_methods_Voucher(voucher: SMDomainVoucher) {
  // }
  ...
  val package_methods_platformmodel_visitor = new PObjectFunction[Unit] {
    override protected def apply_EntityEntity(entity: PEntity) {
    ...
    override protected def apply_ObjectEntity(obj: PObject) {
    }
  }
...
*/
  ...
  protected def package_methods_platform_Entity(entity: PEntity) {
  }

  protected def package_methods_platform_Package(pkg: PPackage) {
  }

  protected def package_methods_Extension {
  }

  protected final def entities_in_module(): Seq[PEntity] = {
    map_entities_in_module {
      case x: PEntity => x
    }
  }

  protected final def map_entities_in_module[T](f: PEntity => T): Seq[T] = {
    pContext.collectPlatform {
      case x: PEntity => f(x)
    }
  }

  protected final def collect_entities_in_module[T](f: PartialFunction[PEntity,T]): Seq[T] = {
    pContext.collectPlatform {
      case x: PEntity => x
    } collect f
  }

  protected final def traverse_entities_in_module[T](f: PEntity => T) {
...
/*
  protected final def package_children_map[T](f: PObject => T): Seq[T] = {
//    pobject.packageChildren.map(f)
  }

  protected final def package_children_collect[T](f: PartialFunction[PObject, T]): Seq[T] = {
//    pobject.packageChildren.collect(f)
  }

  protected final def package_children_entity_map[T](f: PEntity => T): Seq[T] = {
...
*/
  ...
  final protected def doc_attr_name(attr: PAttribute) = {
    pContext.voucherAttributeName(attr)
  }

  final protected def doc_var_name(attr: PAttribute) = {
    pContext.voucherVariableName(attr)
  }
...
//   }
   ...
   */
  protected final def actor_entities: Seq[PEntity] = {
    // collect_entities_in_module {
    //   case x: PEntity if (x.modelObject.isInstanceOf[SMDomainActor]) => x
    // }
    ???
  }

  protected final def role_entities: Seq[PEntity] = {
    // collect_entities_in_module {
    //   case x: PEntity if (x.modelObject.isInstanceOf[SMDomainRole]) => x
    // }
    ???
  }

  protected final def event_entities: Seq[PEntity] = {
    // collect_entities_in_module {
    //   case x: PEntity if (x.modelObject.isInstanceOf[SMDomainEvent]) => x
    // }
    ???
  }

  protected final def resource_entities: Seq[PEntity] = {
    // collect_entities_in_module {
    //   case x: PEntity if (x.modelObject.isInstanceOf[SMDomainResource]) => x
    // }
    ???
  }

  protected final def summary_entities: Seq[PEntity] = {
    // collect_entities_in_module {
    //   case x: PEntity if (x.modelObject.isInstanceOf[SMDomainSummary]) => x
    // }
    ???
  }

  protected final def association_entities: Seq[PEntity] = {
    // collect_entities_in_module {
    //   case x: PEntity if (x.modelObject.isInstanceOf[SMDomainAssociationEntity]) => x
    // }
    ???
  }
...
object NullClassDefinition extends GenericClassDefinition(
  ...
  override def build() {}
  override def toText = "Not implemented yet. override class_Defition or write_Content in PObject."
}
