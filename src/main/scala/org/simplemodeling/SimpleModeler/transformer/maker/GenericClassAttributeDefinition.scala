...
abstract class GenericClassAttributeDefinition(
  val pContext: PContext, // PEntityContext
  val model: PModel,
  ...
  protected final def is_settable(): Boolean = {
    ...
    attr.attributeKind match { // XXX
      ...
      case IdAttributeKind => attr.idPolicy match {
        case AutoIdPolicy => false
        case ApplicationIdPolicy => true
      }
...
//    variable_plain_Attribute_Instance_Variable(persistent_element_type, varName);
  ...
  protected def variable_plain_entity_id_with_association_property(entityType: PEntityType) {
    ...
    entityType.entity.idPolicy match {
      case AutoIdPolicy => variable_plain_entity_key_long(entityType)
      case ApplicationIdPolicy => variable_plain_entity_key_unencoded_string(entityType)
    }
  }
   ...
   * Voucher
   */
  def method_voucher_plain() {
  }

  def method_voucher_shallow() {
  }

  def voucher_methods_update_attribute() {
  }
}
