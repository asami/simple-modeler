...
import org.simplemodeling.SimpleModeler.transformer.maker.expr.SqlExpressionBuilder
...
)(implicit val context: PContext) extends SqlMaker {
...
*/
  ...
  def select = {
    val tablename = context.sqlTableName4SingleTableInheritance(model, entity)
  ...
  private def _columns_join(o: PEntity) = {
  ...
  object EntityReferenceForSuperAssociation {
    ...
    def unapply(attr: PAttribute): Option[List[String]] = {
      ...
      if (attr.associationKind == MAssociation.SuperAssociationKind) {
...
//    println("SqlMaker#_column(%s/%s) = %s".format(entity.name, attr.name, attr))
  ...
  private def _join_association_class_direct(ownattr: PAttribute, t: String, participation: AttributeParticipation, targetattr: PAttribute) = {
    ...
    val tx = t + "x"
    val assoctable = context.sqlTableName4SingleTableInheritance(model, assoc)
  ...
  private def _joins_join_entity(e: PEntity) = {
...
class VoucherSqlMaker(val voucher: PVoucher, isTarget: PAttribute => Boolean = _ => true)(implicit val context: PContext) extends SqlMaker {
  def create = {
    "create"
  }
