...
class BuilderJavaClassAttributeDefinition(
  ...
  jmaker: JavaMaker
) extends JavaClassAttributeDefinition(pContext, model, aspects, attr, owner, jmaker)
  ...
  override protected def variable_plain_entity_attribute(e: PEntityType) {
    ...
    variable_plain_Attribute_Instance_Variable(code_single_voucher_type(e), varName);
  }
  ...
  override protected def method_bean_single_entity(e: PEntityType) {
    ...
    jm_public_get_method(code_single_voucher_type(e), attrName)
    jm_mark("// BuilderJavaClassAttributeDefinition#method_bean_single_entity")
    jm_public_set_method(attrName, code_single_voucher_type(e), paramName, varName)
  }

  override protected def method_bean_multi_entity(e: PEntityType) {
    val elementtype = code_single_voucher_type(e)
    val listtype = code_multi_voucher_type(e)
  ...
  override def method_with_plain_single_entity(e: PEntityType) {
    ...
    jm_public_with_method(owner.name, attrName, code_single_voucher_type(e), paramName, varName)    
  }

  override def method_with_plain_multi_entity(e: PEntityType) {
    val elementtype = code_single_voucher_type(e)
    val containertype = code_multi_voucher_type(e)
