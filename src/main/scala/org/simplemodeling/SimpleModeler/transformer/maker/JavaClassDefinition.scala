...
class JavaClassDefinition(
...
) extends GenericClassDefinition(pContext, model, aspects, pobject) with JavaMakerHolder with JavaClassCodeUtils {
  type ATTR_DEF = JavaClassAttributeDefinition
  type OP_DEF = JavaClassOperationDefinition

  /**
   * References to entity are converted to vouchers of entity.
   */
  def isVoucher = false
  ...
  private lazy val _builder = new BuilderJavaClassDefinition(pContext, model, Nil, pobject, jm_maker)
  ...
  override protected def attribute(attr: PAttribute): ATTR_DEF = {
    new JavaClassAttributeDefinition(pContext, model, aspects, attr, this, jm_maker)
  }

  override protected def operation(op: POperation): OP_DEF = {
    new JavaClassOperationDefinition(pContext, model, aspects, op, this, jm_maker)
  }
...
//    println("JavaClassDefinition#constructors_plain_constructor(%s) = %s".format(this.name, not_derived_whole_attribute_definitions.map(_.attr.name)))
  ...
  override protected def constructors_doc_constructor {
    jm_public_constructor("%s(%s o)", name, voucherName) {
      if (hasBaseObject) {
        jm_pln("super(o);")
      }
      for (a <- not_derived_implements_attribute_definitions) {
        if (aspects.find(_.weaveDocConstructorAttributeBlock(a.attr, a.varName, "o")).isDefined) {}
          ...
          if (a.attr.isMulti) {
            a.attr.attributeType match {
              case t: PEntityType => {
                jm_for(t.entity.voucherName, "elem", in) {
  ...
  protected final def constructors_plain_constructor_for_voucher {
    val params = not_derived_whole_attribute_definitions.filter(!_.isInject).
    map(a => {
      val typename = if (a.isMulti) {
        a.attr.attributeType match {
          case t: PEntityType => {
            val b = pContext.entityVoucherName(t.entity)
            "List<" + b + ">"
          }
          case _ => a.javaType
        }
        ...
        a.attr.attributeType match {
          case t: PEntityType => {
            pContext.entityVoucherName(t.entity)
          }
          case _ => a.javaType
        }
      }
    ...
    jm_mark("// JavaClassDefinition#constructors_plain_constructor_for_voucher")
    jm_public_constructor("%s(%s)", name, params) {
      if (hasBaseObject) {
    ...
    val params = attributeDefinitions.filter(!_.isInject).
      map(a => {
        val typename = a.attr.attributeType match {
          case t: PEntityType => {
            pContext.entityVoucherName(t.entity.modelEntity)
          }
          case _ => a.javaType
        }
        typename + " " + a.paramName 
...
//        }      
  ...
  override protected def voucher_methods_make {
    def attribute_plain(attr: GenericClassAttributeDefinition) {
      attr.method_voucher_plain
    }

    def attribute_shallow(attr: GenericClassAttributeDefinition) {
      attr.method_voucher_shallow
    }

    def voucher_common(methodName: String, makeAttr: GenericClassAttributeDefinition => Unit) {
      jm_public_method("%s %s()".format(voucherName, methodName)) {
        jm_var(voucherName + ".Builder", "doc", "%s.Builder()", voucherName)
    ...
    def voucher_plain {
      voucher_common("make_voucher", attribute_plain)
    }

    def voucher_shallow {
      voucher_common("make_voucher_shallow", attribute_shallow)
    }
  ...
  override protected def voucher_methods_update {
    jm_public_void_method("update_voucher(%s doc)", voucherName) {
...
*/
      for (a <- attributeDefinitions if !a.attr.isId) {
        a.voucher_methods_update_attribute
      }
    }
  }
