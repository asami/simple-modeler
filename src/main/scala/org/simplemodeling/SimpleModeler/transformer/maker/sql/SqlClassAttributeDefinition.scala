...
abstract class SqlClassAttributeDefinition(
  ...
  maker: JavaMaker) extends GenericClassAttributeDefinition(sqlContext, model, aspects, attr, owner) with JavaMakerHolder {
  ...
  protected val sql_typename: PObjectTypeFunction[String] = new PObjectTypeFunction[String] {
    ...
    override protected def apply_ObjectIdType(datatype: PObjectIdType): String =  {
      "VARCHAR(128)"
    }
    ...
    override protected def apply_VoucherType(datatype: PVoucherType): String = {
      handle_model(datatype)
    }
  ...
  private def _datatype = {
    attr.getSqlDatatypeName getOrElse attr.attributeType(sql_typename)
  }
}
...
class NullSqlClassAttributeDefinition(
  ...
  owner: SqlClassDefinition,
  maker: JavaMaker
) extends SqlClassAttributeDefinition(pContext, model, aspects, attr, owner, maker)
