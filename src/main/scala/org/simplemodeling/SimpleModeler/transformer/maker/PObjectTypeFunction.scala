...
trait PObjectTypeFunction[T] extends PartialFunction[PObjectType, T] {
  ...
  override def apply(objecttype: PObjectType): T = {
    objecttype match {
      ...
      case ot: PObjectIdType => apply_ObjectIdType(ot)
      ...
      case ot: PVoucherType => apply_VoucherType(ot)
...
*/
  ...
  protected def apply_ObjectIdType(datatype: PObjectIdType): T = {
    handle_auxiliary(datatype)
  }
  ...
  protected def apply_VoucherType(datatype: PVoucherType): T = {
    handle_model(datatype)
  }
